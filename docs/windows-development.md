...
4. Confirme que nenhum listener residual permanece ativo na porta `5433`:
   ...
   ```
5. Somente apÃ³s a liberaÃ§Ã£o da porta, inicie o serviÃ§o novamente:
   ```powershell
...
6. Confirme que o status retornou para `Running`:
   ...
   Get-Service postgresql-x64-18
   ```
7. Valide os sockets loopback:
   ```powershell
   Get-NetTCPConnection -LocalPort 5433 | Select-Object LocalAddress, LocalPort, State
   ```
