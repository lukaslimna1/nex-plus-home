...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
    ...
    -- 2. ATTEMPT HEADS (Projeção Operacional Mutável)
    ...
    -- 5. EVIDENCE <-> SIGNALS (Relacional Append-Only)
    CREATE TABLE IF NOT EXISTS "nex_execution_evidence_signals" (
      ...
      CONSTRAINT "nex_evi_sig_unique" UNIQUE ("evidence_id", "signal_id"),
    ...
    -- 7. OUTCOME <-> EVIDENCE (Relacional Append-Only)
    CREATE TABLE IF NOT EXISTS "nex_execution_outcome_evidence" (
      ...
      CONSTRAINT "nex_out_evi_unique" UNIQUE ("assessment_id", "evidence_id"),
    ...
    -- 8. OUTCOME HEADS (Projeção Operacional Mutável, 1 row por attempt)
    CREATE TABLE IF NOT EXISTS "nex_execution_outcome_heads" (
      ...
      "latest_assessment_id" varchar NOT NULL REFERENCES "nex_execution_outcome_assessments"("assessment_id") ON DELETE RESTRICT,
    ...
    CREATE TABLE IF NOT EXISTS "nex_execution_receipts" (
      ...
      "attempt_id" varchar REFERENCES "nex_execution_attempt_heads"("attempt_id") ON DELETE RESTRICT,
