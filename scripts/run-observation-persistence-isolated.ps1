...
try {
    ...
    # Ajustar ledger para manter foundation como Batch 1, multiuser_auth como Batch 2 e observation_persistence como Batch 3
    ...
    # 7. Testar Migration DOWN (Rollback da migration 0.85B)
    Write-Host "`n[4/6] Testando rollback de migration (DOWN do 0.85B) no banco descartável..." -ForegroundColor Yellow
    & npx payload migrate:down
    if ($LASTEXITCODE -ne 0) { throw "Falha ao executar payload migrate:down para 0.85B no banco descartável" }
